...
function isPokemonInSelectedGens(pokemon, pokemonData = null, gensOverride = null) {
  ...
  const isNearDuplicateForm =
    rawName.includes('-totem') ||
    rawName.includes('-busted') ||
    rawName.includes('-gulping') ||
    rawName.includes('-gorging');
  if (isNearDuplicateForm) {
    return false;
  }
...
function attachAutocompleteKeyboardNav(inputField, suggestionsContainer) {
  let activeIndex = -1;
...
function resetTimedToStartScreen() {
  ...
  timedStatsLabel.textContent = 'Score: 0 - Correct: 0';
...
function endTimedGame() {
  ...
  timedFinalScoreText.textContent = `Final Score: ${timedScore} (${timedCorrectCount} correct in ${timedDuration}s)`;
