  ...
  function renderVsCanvas(img, mode, progress) {
    ...
    ctx.clearRect(0, 0, w, h);
     ...
     if (mode === 'Classic Blur') {
      // iOS Safari doesn't support ctx.filter — use a CSS filter on the
      // canvas element itself. Identical visual result, works everywhere.
      vsRevealCanvas.style.filter = `blur(${(1 - progress) * 20}px)`;
      ctx.drawImage(img, 0, 0, w, h);
      return;
    }

    // Clear any leftover CSS blur when a non-Classic mode is drawn
    // (e.g. Classic Blur one round, Pixelation the next).
    vsRevealCanvas.style.filter = 'none';
    ...
    if (mode === 'Pixelation') {
      ...
      // rectangles line up with the visible canvas. Without this we'd be
      // sampling a 70x70 corner of the artwork's natural (e.g. 475x475)
      // size and stretching it — the same "zoomed in" bug Pixelation had.
      if (!vsScaledSprite) {
